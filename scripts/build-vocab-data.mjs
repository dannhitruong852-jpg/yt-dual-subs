...
function parseCefr(file) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const lemma = validLemma(headword);
    const level = mapping[String(cefr || '').trim().toUpperCase()];
    if (!lemma || !level || headword.includes(' ')) continue;
    const previous = levels.get(lemma);
    if (!previous || level < previous) levels.set(lemma, level);
  }
  return levels;
}
...
function parseEcdict(file, levels, zh) {
  ...
  const wordAt = header.indexOf('word');
  const tagAt = header.indexOf('tag');
  const translationAt = header.indexOf('translation');
  for (const line of rows) {
    if (!line || line.includes('"')) continue; // full CSV support is optional; skip complex rows conservatively
    const cols = line.split(',');
    const lemma = validLemma(cols[wordAt]);
    if (!lemma) continue;
    const tags = String(cols[tagAt] || '').toLowerCase();
    let level = 0;
    if (/\b(cet6|ky)\b/.test(tags)) level = 5;
    if (/\b(ielts|toefl)\b/.test(tags)) level = 6;
    if (/\bgre\b/.test(tags)) level = 7;
    if (level) levels.set(lemma, Math.max(levels.get(lemma) || 0, level));
    if (level >= 5 && translationAt >= 0) {
      const candidates = (String(cols[translationAt] || '').match(/[\p{Script=Han}]{2,6}/gu) || []).slice(0, 4);
      if (candidates.length) zh.set(lemma, [...new Set(candidates)]);
    }
  }
}
...
function main() {
  ...
  if (!args.cefr || !args.out) {
    console.error('Usage: node scripts/build-vocab-data.mjs --cefr <word_list_cefr.csv> [--ecdict <ecdict.csv>] --out <vocab-data.js>');
    process.exit(2);
  }
  const levels = parseCefr(args.cefr);
  const zh = new Map();
  ...
  fs.writeFileSync(args.out, emit({ version: 2, levels: levelObject, zh: zhObject }));
